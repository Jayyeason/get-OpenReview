...
# ============================================================
# 打包 conda dpo 环境为 .tar 镜像
# 使用方法：
#   bash pack_dpo_env.sh [output_path]
# ============================================================
...
OUTPUT_FILE="${OUTPUT_PATH}/${ENV_NAME}_env_$(date +%Y%m%d_%H%M%S).tar.gz"
...
if [ $? -eq 0 ]; then
    ...
    echo "恢复环境的方法："
    echo "1. 解压到目标位置："
    echo "   tar -xzf ${OUTPUT_FILE} -C /path/to/miniconda3/envs/"
    echo ""
    echo "2. 或者解压到自定义位置后，创建软链接："
    echo "   tar -xzf ${OUTPUT_FILE} -C /custom/path/"
