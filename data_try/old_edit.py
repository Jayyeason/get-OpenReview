...
OUT_DIR = "/remote-home1/bwli/get_open_review/data_try/iclr2025_forums"  # 每篇论文一个 <forum>.json
...
def extract_reviewer_id(signatures: List[str]) -> Optional[str]:
    sigs = signatures or []
    for s in sigs:
        if "Reviewer" in s:
            return s.split("/")[-1]
...
def build_per_reviewer_chains(
    ...
    for root_review in review_roots:
        short_id = extract_reviewer_id(root_review.get("signatures") or [])
        if not short_id:
            continue
        per_reviewer.setdefault(short_id, []).append(root_review)
        note_to_reviewer[root_review["note_id"]] = short_id

    for event in events:
        if event.get("event_type") == "review_version":
            note_id = event["note_id"]
            rid = note_to_reviewer.get(note_id)
            if rid and event not in per_reviewer.get(rid, []):
                per_reviewer[rid].append(event)
    ...
    for event in events:
        ...
        rid = note_to_reviewer.get(note_id)
        if rid:
            per_reviewer.setdefault(rid, []).append(event)
